...
export default function BoostersModal({ isOpen, onClose, user }: BoostersModalProps) {
  ...
  // Fetch user's boosters/upgrades
  const { data: boosters = [], isLoading } = useQuery({
    queryKey: ["/api/upgrades", user.id],
    queryFn: () => apiRequest("GET", `/api/upgrades/${user.id}`).then(res => res.json()),
  });
  ...
  const createBoosterMutation = useMutation({
    mutationFn: async (boosterData: any) => {
      const response = await apiRequest("POST", "/api/admin/upgrades", boosterData);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/upgrades", user.id] });
  ...
  const deleteBoosterMutation = useMutation({
    mutationFn: async (boosterId: string) => {
      const response = await apiRequest("DELETE", `/api/admin/upgrades/${boosterId}`);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/upgrades", user.id] });
  ...
  const canAfford = (booster: any) => {
    const cost = calculateCost(booster.cost || booster.baseCost || 100, booster.level || 0);
  ...
  const handleDeleteBooster = (boosterId: string) => {
    if (confirm("Are you sure you want to delete this booster?")) {
      deleteBoosterMutation.mutate(boosterId);
    }
  };
              ...
              <ScrollArea className="h-full pr-4">
                <div className="grid gap-4 md:grid-cols-2 pb-4">
                  {boosters.map((booster: any) => {
                    ...
                    const cost = calculateCost(booster.cost || booster.baseCost || 100, booster.level || 0);
                    const hourlyBonus = calculateHourlyBonus(booster.hourlyBonus || 10, booster.level || 0);
                    const tapBonus = calculateTapBonus(booster.tapBonus || 0, booster.level || 0);
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="text-xs text-gray-500 mt-1">
                                Cost: {booster.cost || booster.baseCost} | Hourly: +{booster.hourlyBonus} | Max Lv: {booster.maxLevel}
                              </div>
                            </div>
                            <Button
